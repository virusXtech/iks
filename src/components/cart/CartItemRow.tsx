    ...
    <div className="flex flex-col sm:flex-row items-center justify-between py-4 border-b border-border last:border-b-0 gap-4 sm:gap-0">
      <div className="flex items-center gap-4 w-full sm:w-auto">
        ...
        <div className="flex-grow">
          <h3 className="text-lg font-semibold font-serif text-primary">{item.name}</h3>
          <p className="text-sm text-muted-foreground">€{item.price.toFixed(2)} each</p>
        </div>
      </div>
      <div className="flex items-center gap-2 sm:gap-4">
        <div className="flex items-center gap-1 sm:gap-2">
          <Button variant="outline" size="icon" onClick={() => updateItemQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1}>
          ...
          <Input 
            ...
            onBlur={(e) => { // Ensure onBlur sets to 1 if input is empty or invalid
