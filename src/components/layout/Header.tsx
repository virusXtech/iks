...
import { ShoppingCart, Utensils, Search, Mail, List, Menu as MenuIcon } from 'lucide-react'
    ...
    <header className="bg-card shadow-md sticky top-0 z-50">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        <Link href="/" className="flex items-center gap-2 group">
          ...
          />
          <div>
            <h1 className="text-2xl font-bold text-primary">{RESTAURANT_NAME}</h1>
            <p className="text-xs text-muted-foreground">{RESTAURANT_SUBTITLE}</p>
          </div>
        </Link>
        ...
        <nav className="hidden sm:flex items-center gap-2 sm:gap-3">
          ...
          <Link href="/cart" legacyBehavior passHref>
            <Button variant="outline" className="hover:bg-primary/90 hover:text-primary-foreground relative text-sm">
                ...
                <SheetTitle>
                  <SheetClose asChild>
                    <Link href="/" className="flex items-center gap-2 text-left">
                      ...
                      <span className="text-xl font-bold text-primary">{RESTAURANT_NAME}</span>
                    </Link>
                  </SheetClose>
                </SheetTitle>
              </SheetHeader>
              <div className="flex flex-col gap-4">
                ...
                {navLinks.map(link => (
                  <SheetClose key={link.href} asChild>
                    ...
                    >
                      <link.icon className="h-6 w-6" /> {/* Changed from h-5 w-5 */}
                      {link.label}
                    </Link>
                  </SheetClose>
                ))}
                ...
                <SheetClose asChild>
                  <Link
                    ...
                    className="flex items-center gap-3 p-3 rounded-md hover:bg-accent/80 transition-colors text-foreground hover:text-accent-foreground font-medium" /* Added font-medium */
                  >
                    <ShoppingCart className="h-6 w-6" /> {/* Changed from h-5 w-5 */}
                    Cart
                    {cartItemCount > 0 && (
