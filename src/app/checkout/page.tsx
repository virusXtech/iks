...
export default function CheckoutPage() {
  ...
  if (itemCount === 0 && !isSubmitting) { // Prevent redirect if submitting and cart clears
     return (
  ...
  const handleCheckoutSubmit = async (data: CheckoutFormData) => {
    ...
    } else if (data.serviceType === 'self-service') {
    ...
    clearCart();
    router.push(`/confirmation/${orderId}?name=${encodeURIComponent(data.name)}&serviceType=${data.serviceType}${data.tableNumber ? `&tableNumber=${encodeURIComponent(data.tableNumber)}` : ''}`);
  };
